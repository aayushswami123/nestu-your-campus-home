  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      >
        ...
        <path
          d="M19 17 L19 38 Q19 52 32 52 Q45 52 45 38 L45 17 L38 17 L38 38 Q38 45 32 45 Q26 45 26 38 L26 17 Z"
          fill="#F2ECE2"
        />
...
const INPUT =
  "h-11 rounded-[10px] border border-[#E8E3D7] bg-[#FAFAF7] px-3.5 text-sm text-[#0E0F0C] outline-none transition placeholder:text-[#95968F] focus:border-[#C4622D] focus:ring-4 focus:ring-[#C4622D]/18 focus:bg-white";
...
function WaitlistForm({ onSuccess }: { onSuccess: (r: BackendResult) => void }) {
  ...
  return (
    <form
      onSubmit={submit}
      className="rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-8 shadow-[0_1px_0_rgba(14,15,12,.05),0_18px_40px_-16px_rgba(14,15,12,.14)] relative overflow-hidden"
      style={{
        background:
          "linear-gradient(135deg, rgba(196,98,45,.04), transparent 40%, rgba(92,122,90,.04))",
      }}
    >
      {/* Google */}
      <button
      ...
      >
        <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden>
          ...
          <path
            fill="#FBBC05"
            d="M5.84 14.1A6.99 6.99 0 0 1 5.46 12c0-.73.13-1.44.38-2.1V7.07H2.18A11 11 0 0 0 1 12c0 1.78.43 3.46 1.18 4.93l3.66-2.83z"
          />
          <path
            fill="#EA4335"
            d="M12 4.78c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 1.46 14.97.5 12 .5 7.7.5 3.99 2.97 2.18 6.57l3.66 2.83C6.71 6.71 9.14 4.78 12 4.78z"
          />
        </svg>
      ...
      <div className="my-4 flex items-center gap-3.5 text-[11px] uppercase tracking-[0.14em] text-[#95968F]">
        <span className="h-px flex-1 bg-[#E8E3D7]" /> or with email{" "}
        <span className="h-px flex-1 bg-[#E8E3D7]" />
      </div>
      ...
      <div className={FIELD}>
        ...
        <input
          ...
          className={INPUT}
        />
        <span className="text-[12px] text-[#95968F]">
      ...
      <div className={FIELD}>
        <label className={LABEL}>
          City or university <span className={OPT}>optional</span>
        </label>
        <input
          type="text"
          value={location}
          onChange={(e) => setLocation(e.target.value)}
          placeholder="e.g. UC Berkeley, Austin TX, New York"
          className={INPUT}
        />
      </div>
      ...
      <div className={FIELD}>
        ...
        <input
          ...
          placeholder="NESTU-XXXXXX"
          className={`${INPUT} font-mono tracking-[0.08em] text-[13px]`}
        />
      </div>

      {error && (
        <div className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-[13px] text-red-700">
          {error}
        </div>
      )}
      ...
      >
        {loading ? (slow ? "Almost there…" : "Joining…") : "Get early access"}{" "}
...
function GoogleDetailsForm({
  ...
  email: string;
  name: string;
  onSuccess: (r: BackendResult) => void;
}) {
  const [role, setRole] = useState("");
  const [location, setLocation] = useState("");
  const [referredBy, setReferredBy] = useState("");
  ...
  return (
    <form
      onSubmit={submit}
      className="rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-8 shadow-[0_1px_0_rgba(14,15,12,.05),0_18px_40px_-16px_rgba(14,15,12,.14)]"
    >
...
function SuccessState({ referralCode, position }: { referralCode?: string; position?: number }) {
  ...
  const waText = encodeURIComponent(
    `Just joined the NestU waitlist — verified student housing, no scams: ${fullLink}`,
  );
  ...
  return (
    <div className="rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-8 shadow-[0_1px_0_rgba(14,15,12,.05),0_18px_40px_-16px_rgba(14,15,12,.14)] text-center">
      ...
      {position != null && (
        <p className="mt-1 font-serif text-[22px] text-[#C4622D]">You're #{position} in line.</p>
      )}
      <p className="mt-2 text-sm text-[#6B6C66] max-w-[36ch] mx-auto">
        Share your link below to move up the list when we launch.
      </p>

      {referralCode && (
        <>
          <div className="mt-5 rounded-xl border border-dashed border-[#D9D3C3] bg-[#FAFAF7] p-4">
            <div className="text-[11px] uppercase tracking-[0.14em] text-[#95968F]">
              Your referral code
            </div>
            <div className="mt-1 font-mono text-[22px] font-semibold tracking-[0.14em] text-[#C4622D]">
              {referralCode}
            </div>
          </div>
          <div className="mt-3.5 flex items-center gap-2 rounded-full border border-[#E8E3D7] bg-[#FAFAF7] p-1.5 pl-4">
            <span className="flex-1 truncate text-left font-mono text-[13px] text-[#2A2B27]">
              {link}
            </span>
            <button
              onClick={copy}
              className="flex shrink-0 items-center gap-1.5 rounded-full bg-[#0E0F0C] px-4 py-2 text-xs font-medium text-[#FAFAF7] transition hover:opacity-80 active:scale-95"
            >
              {copied ? (
          ...
          <div className="mt-3 grid grid-cols-2 gap-2.5">
            ...
            <button
              onClick={copy}
              className="flex items-center justify-center gap-2 rounded-[10px] border border-[#E8E3D7] bg-[#FAFAF7] px-4 py-3 text-sm font-medium text-[#0E0F0C] transition hover:bg-[#F1EEE6] active:scale-[0.98]"
            >
...
function OAuthError({ onBack }: { onBack: () => void }) {
  return (
    <div className="rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-8 shadow-[0_1px_0_rgba(14,15,12,.05),0_18px_40px_-16px_rgba(14,15,12,.14)] text-center space-y-4">
      ...
      <p className="text-sm text-[#6B6C66]">
        Google sign-in worked but we couldn't save your spot. Please try the email form.
      </p>
      <button
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-[10px] bg-[#C4622D] px-5 py-3 text-sm font-medium text-white transition hover:opacity-90"
      >
        Use email instead
      </button>
    </div>
        ...
        <div className="grid items-center gap-10 lg:gap-16 lg:grid-cols-[1.15fr_1fr]">
          ...
          <div data-reveal>
            ...
            <h1
              data-reveal
              data-delay="1"
              className="mt-6 font-serif text-[clamp(44px,7.4vw,88px)] leading-[1.0] tracking-[-0.025em] text-[#0E0F0C] max-w-[16ch]"
            >
              Housing shouldn't be a <em className="italic text-[#C4622D]">fairy tale.</em>
              <br />
            ...
            <p
              data-reveal
              data-delay="2"
              className="mt-5 max-w-[56ch] text-[18px] leading-[1.55] text-[#6B6C66]"
            >
              Stop playing roommate roulette. Stop sending deposits to strangers. Verified
              subleases, AI roommate matching, and real neighborhood data — built for students,
              interns, and international newcomers.
            </p>
            <div data-reveal data-delay="2" className="mt-8">
              <button
                onClick={() => scrollTo("how")}
                className="flex items-center gap-2 rounded-[12px] border border-[#E8E3D7] bg-transparent px-5 py-3.5 text-[15px] font-medium text-[#0E0F0C] transition hover:bg-[#F1EEE6] hover:border-[#D9D3C3] active:scale-[0.98]"
              >
            ...
            <div
              data-reveal
              data-delay="3"
              className="mt-5 flex flex-wrap gap-4 text-[13px] text-[#6B6C66]"
            >
          ...
          <div data-reveal data-delay="2">
            ...
            {pageState === "google-details" && (
              <GoogleDetailsForm email={googleEmail} name={googleName} onSuccess={onSuccess} />
            )}
            {pageState === "success" && (
              <SuccessState
                referralCode={successData?.referralCode}
                position={successData?.position ?? undefined}
              />
            )}
  ...
  return (
    <section className="border-y border-[#E8E3D7] bg-[#F1EEE6]">
      <div className="mx-auto flex max-w-[1180px] flex-col sm:flex-row sm:flex-wrap items-center justify-center gap-2 sm:gap-6 px-4 sm:px-7 py-5 text-[#6B6C66] text-center">
        <span className="text-[11px] uppercase tracking-[0.14em] text-[#95968F]">
          Building together
        </span>
        <span className="font-serif text-[18px] italic text-[#2A2B27]/70">
        ...
        <span className="hidden sm:block h-1.5 w-1.5 rounded-full bg-[#C4622D]" />
        <span className="font-serif text-[18px] italic text-[#2A2B27]/70">
          students, interns, internationals & landlords
        </span>
      </div>
    </section>
  );
  ...
  return (
    <section className="border-y border-[#E8E3D7] bg-[#F1EEE6] py-10 md:py-16">
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <div
          data-reveal
          className="mx-auto max-w-[880px] rounded-[22px] border border-[#E8E3D7] bg-white p-6 sm:p-10 shadow-[0_1px_0_rgba(14,15,12,.04),0_8px_24px_-10px_rgba(14,15,12,.08)] relative overflow-hidden md:p-14"
        >
          <p className="font-serif text-[clamp(24px,3.4vw,36px)] leading-[1.25] tracking-[-0.012em] text-[#0E0F0C]">
            <span
              className="font-serif text-[90px] leading-[0] text-[#C4622D] mr-2"
              style={{ verticalAlign: "-28px" }}
            >
              "
            </span>
            NestU is what happens when someone finally listens to students. Verified listings, real
            roommate compatibility, no broker fees — it's the housing search we all should've had
            years ago.
          </p>
          <div className="mt-8 flex items-center gap-3.5 border-t border-[#E8E3D7] pt-6">
            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#0E0F0C] text-[13px] font-semibold text-[#FAFAF7]">
              SR
            </span>
            <div>
            ...
            <span className="ml-auto hidden font-serif text-[18px] text-[#2A2B27]/70 sm:block">
              — Joined #0042
            </span>
          </div>
        </div>
  ...
  return (
    <section className="py-14 md:py-24">
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <div
          data-reveal
          className="sublease-card relative grid gap-9 overflow-hidden rounded-[22px] border border-[#E8E3D7] bg-white p-6 sm:p-10 shadow-[0_1px_0_rgba(14,15,12,.04),0_8px_24px_-10px_rgba(14,15,12,.08)] lg:grid-cols-[1.05fr_.95fr] lg:gap-14 lg:p-14 items-center"
        >
          <div
            className="absolute -right-28 -top-28 h-72 w-72 rounded-full"
            style={{
              background: "radial-gradient(circle, rgba(196,98,45,.18), transparent 65%)",
              pointerEvents: "none",
            }}
          />
          <div className="relative z-10">
            <span className="text-[12px] uppercase tracking-[0.16em] text-[#C4622D] font-medium">
            ...
            <p className="mt-4 text-[16px] leading-[1.6] text-[#6B6C66] max-w-[56ch]">
              Studying abroad, graduating early, or just leaving for the summer? List your place in
              60 seconds and get matched to verified students looking right now — no Facebook
              groups, no flaky DMs, no broker fees. Free to post, always.
            </p>
            <ul className="mt-5 grid gap-2.5">
              {[
                "Post in under a minute",
                "We screen every applicant",
                "Free — no listing fees, ever",
              ].map((t) => (
          ...
          <div className="sublease-mock relative z-10 flex flex-col gap-3.5 rounded-[18px] border border-[#E8E3D7] bg-[#FAFAF7] p-5 shadow-[0_1px_0_rgba(14,15,12,.05),0_18px_40px_-16px_rgba(14,15,12,.14)]">
            <div className="flex items-center gap-2.5 border-b border-[#E8E3D7] pb-2">
              <span className="flex gap-1">
                <i className="block h-2 w-2 rounded-full bg-[#D9D3C3]" />
                <i className="block h-2 w-2 rounded-full bg-[#D9D3C3]" />
                <i className="block h-2 w-2 rounded-full bg-[#D9D3C3]" />
              </span>
            ...
            {[
              ["Address", "123 Telegraph Ave, Berkeley"],
              ["Monthly rent", "$890 / mo"],
            ].map(([label, val]) => (
              <div key={label} className="flex flex-col gap-1">
                <label className="text-[11px] uppercase tracking-[0.1em] text-[#6B6C66]">
                  {label}
                </label>
                <div className="rounded-[10px] border border-[#E8E3D7] bg-white px-3 py-2.5 text-[13px] font-medium text-[#0E0F0C]">
                  {val}
                </div>
              </div>
            ))}
    ...
    <section className="py-14 md:py-24">
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <div className="section-head">
          <span
            data-reveal
            className="text-[12px] uppercase tracking-[0.16em] text-[#C4622D] font-medium"
          >
            The problem
          </span>
          <h2
            data-reveal
            data-delay="1"
            className="mt-3 font-serif text-[clamp(32px,5vw,56px)] leading-[1.05] tracking-[-0.02em] text-[#0E0F0C] max-w-[22ch]"
          >
        ...
        <div
          data-reveal
          data-delay="2"
          className="mt-10 grid gap-8 sm:gap-11 rounded-[22px] border border-[#E8E3D7] bg-white p-6 sm:p-11 md:grid-cols-3"
        >
          <div>
            <div className="font-serif text-[clamp(48px,6vw,72px)] leading-[0.95] text-[#C4622D] italic">
              <CountUp to={72} suffix="%" />
            </div>
            <p className="mt-2.5 text-sm leading-[1.55] text-[#6B6C66] max-w-[28ch]">
              of students say finding housing near campus is stressful.
            </p>
          </div>
          <div>
            <div className="font-serif text-[clamp(48px,6vw,72px)] leading-[0.95] text-[#0E0F0C]">
              1<span className="text-[#C4622D]">:</span>3
            </div>
            <p className="mt-2.5 text-sm leading-[1.55] text-[#6B6C66] max-w-[28ch]">
              students have been scammed or nearly scammed on Craigslist.
            </p>
          </div>
          <div>
            <div className="font-serif text-[clamp(48px,6vw,72px)] leading-[0.95] text-[#0E0F0C]">
            ...
            <p className="mt-2.5 text-sm leading-[1.55] text-[#6B6C66] max-w-[28ch]">
              average broker fee just to view an apartment.
            </p>
          </div>
        </div>
      </div>
...
function HowItWorks() {
  const steps = [
    ...
    },
    {
      ...
      d: "Filter by price, distance to campus, furnishing, pets. Every listing is landlord-verified before going live.",
    },
    {
      n: "03",
      t: "Match and move in",
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-3">
          {steps.map((s, i) => (
            <div
              ...
              className="card-hover rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-7"
            >
              <div className="font-serif text-[22px] tracking-[0.02em] text-[#C4622D]">{s.n}</div>
              <h3 className="mt-3.5 font-serif text-[26px] leading-[1.1] tracking-[-0.012em] text-[#0E0F0C]">
                {s.t}
              </h3>
  ...
  const features = [
    {
      ...
      body: "Every landlord goes through identity verification. Every listing is reviewed before it goes live. No Craigslist risk, no fake posts, no lockbox tricks.",
      tags: ["Identity verified", "Short-term ok", "Furnished options", "Semester leases"],
      span: true,
    },
    {
      icon: SparkIcon,
      title: "Roommate matching that works.",
      body: "Answer 8 questions. Our AI scores compatibility on sleep, cleanliness, budget, and lifestyle.",
      tags: ["Sleep", "Cleanliness", "Budget"],
      span: false,
    },
    {
      icon: PinIcon,
      title: "Know the block before you sign.",
      body: "Walkability to campus, food, transit, gyms, grocery. Decide with full context, not just rent.",
      tags: ["Walkability", "Food nearby", "Transit"],
      span: false,
    },
    {
      icon: UsersIcon,
      title: "Move in with people you click with.",
      body: "Group chat with matches, share listings, coordinate move-in dates — all in one place before lease day.",
  ...
  return (
    <section id="features" className="py-14 md:py-24">
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <span
          data-reveal
          className="text-[12px] uppercase tracking-[0.16em] text-[#5C7A5A] font-medium"
        >
          Features
        </span>
        <h2
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-2">
          {features.map((f) => (
            <div
              key={f.title}
              data-reveal
              className={`card-hover rounded-[22px] border border-[#E8E3D7] bg-white p-5 sm:p-9 ${f.span ? "md:col-span-2" : ""}`}
            >
              <div className="mb-6 flex h-11 w-11 items-center justify-center rounded-[12px] bg-[#F1EEE6] text-[#0E0F0C]">
                <f.icon className="h-5 w-5" />
              </div>
              <h3 className="font-serif text-[30px] leading-[1.05] tracking-[-0.012em] text-[#0E0F0C]">
                {f.title}
              </h3>
              ...
              <div className="mt-5 flex flex-wrap gap-1.5">
                {f.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded-full border border-[#E8E3D7] bg-[#F1EEE6] px-2.5 py-1 text-[12px] text-[#2A2B27]"
                  >
                    {t}
                  </span>
                ))}
...
function Marquee() {
  const items = [
    "Verified students & professionals",
    "AI roommate matching",
    "Real subleases, zero scams",
    ...
    <div className="marquee-dark">
      <div className="marquee-track flex gap-9 pr-9 shrink-0">
        {doubled.map((it, i) => (
          <span
            key={i}
            className="flex shrink-0 items-center gap-2.5 text-sm font-medium text-[#FAFAF7] whitespace-nowrap"
          >
            {it}
      ...
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <div className="mx-auto max-w-[720px] text-center">
          ...
          <h2
            data-reveal
            data-delay="1"
            className="mt-3 font-serif text-[clamp(32px,5vw,56px)] leading-[1.05] tracking-[-0.02em] text-[#0E0F0C]"
          >
          ...
          <p
            data-reveal
            data-delay="2"
            className="mt-3.5 text-[17px] leading-[1.55] text-[#6B6C66]"
          >
            Join the waitlist and tell us where you study — the most-requested campuses launch
            first.
          </p>
        </div>
        <div data-reveal data-delay="2" className="mt-10 flex justify-center">
          <div className="inline-flex flex-wrap items-center justify-center gap-4 rounded-[22px] border border-[#E8E3D7] bg-white px-7 py-5 shadow-[0_1px_0_rgba(14,15,12,.04),0_8px_24px_-10px_rgba(14,15,12,.08)]">
            ...
            <span className="font-serif text-[22px] tracking-[-0.012em] text-[#0E0F0C]">
              Coming soon
            </span>
      ...
      <div className="mx-auto max-w-[1180px] px-4 sm:px-7">
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.4fr]">
          ...
          <div>
            ...
            <a
              href="mailto:nestu-waitlist@aayushswami.com"
              className="mt-5 inline-block text-[13px] text-[#C75A38] hover:opacity-80 transition-opacity"
            >
          ...
          {/* Product */}
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-[#FAFAF7]/35 mb-5">
              Product
            </p>
            <ul className="space-y-3.5">
              {[
                ...
                <li key={id}>
                  <button
                    onClick={() => scrollTo(id)}
                    className="text-[14px] text-[#FAFAF7]/60 hover:text-[#FAFAF7] transition-colors text-left"
                  >
                    {label}
                  </button>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-3.5">
              {[
                ...
                <li key={t} className="text-[14px] text-[#FAFAF7]/60">
                  {t}
                </li>
              ))}
            </ul>
        ...
        <div className="mt-14 border-t border-white/8 pt-6 flex flex-wrap items-start justify-between gap-3 text-[12px] text-[#FAFAF7]/35">
          ...
          <p className="max-w-[56ch] leading-[1.6]">
            NestU is a housing search platform, not a licensed real estate broker. Joining the
            waitlist does not constitute a lease, reservation, or commitment of any kind.
          </p>
        </div>
      </div>
    </footer>
...
export default function App() {
  ...
  const handleSuccess = (result: BackendResult) => {
